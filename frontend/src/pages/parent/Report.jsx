...
import { FileText, CheckCircle, User, Moon, BookOpen, Activity } from 'lucide-react';
...
const ParentReport = () => {
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-8">
      {/* Header */}
      <div>
        <h1 className="text-3xl font-bold text-slate-900 tracking-tight">Parent Report</h1>
        <p className="text-slate-500 mt-1">Mental health status overview for your child.</p>
      </div>
      ...
      <div className="bg-emerald-50 border border-emerald-200 rounded-lg p-4 flex items-center">
        ...
        <div>
          ...
          <p className="text-xs text-emerald-600">Reviewed by {report.counselorName} on {report.reportDate}</p>
        </div>
      </div>
        ...
        <div className="bg-white rounded-lg shadow-sm border border-slate-200 p-6">
          ...
          <div className="space-y-4">
            <div className="flex justify-between items-center border-b border-slate-50 pb-2">
              <span className="text-slate-600 text-sm">Mood Stability</span>
              <span className="font-semibold text-emerald-600 text-sm">{report.metrics.mood}</span>
            </div>
            <div className="flex justify-between items-center border-b border-slate-50 pb-2">
              <span className="text-slate-600 text-sm">Stress Level</span>
              <span className="font-semibold text-amber-600 text-sm">{report.metrics.stress}</span>
            </div>
            <div className="flex justify-between items-center">
              <span className="text-slate-600 text-sm">Sleep Quality</span>
              <span className="font-semibold text-red-500 text-sm">{report.metrics.sleep}</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="text-center text-xs text-slate-400 mt-8">
        <p>Confidential Document • Not for detailed diagnosis</p>
      </div>
    </div>
  );
};
